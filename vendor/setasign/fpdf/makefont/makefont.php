...
function MakeFont($fontfile, $enc='cp1252', $dist = '', $embed=true, $subset=true)
{
	...
	if($embed)
	{
		...
		{
			$file = $basename.'.z';
			SaveToFile( trailingslashit( $dist ) . $basename . '.z', gzcompress($info['Data']), 'b');
			$info['File'] = $file;
	...
	MakeDefinitionFile( trailingslashit( $dist ) . $basename . '.php', $type, $enc, $embed, $subset, $map, $info);
